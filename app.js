// JavaScript Logic - Quranic IQ Academy
...
// 3. Course Curriculums Dynamic Modal
...
function initCourseModal() {
  ...
  const triggerBtns = document.querySelectorAll('.btn-curriculum');
...
function initCalculator() {
  ...
  function updatePricing() {
    ...
    // Calculate total lessons per month (approx 4.33 weeks per month)
    const monthlyLessons = Math.round(classesPerWeek * 4.33);
    totalLessons.textContent = `${monthlyLessons} classes`;

    // Apply modifiers based on duration
...
function initTutorSlider() {
  ...
  // Dynamic Audio Recitation Simulation
  function playMelodiousRecitation(type) {
    ...
    // Synthesize a beautiful harmonic chord sweep matching classical chants
    // Generate different chords depending on tutor credentials
    const chordRoots = {
      tutor1: [196.00, 293.66, 392.00, 493.88], // G Major (Warm and Grand)
      tutor2: [220.00, 329.63, 440.00, 523.25], // A Minor (Soft and Sweet)
      tutor3: [174.61, 261.63, 349.23, 440.00]  // F Major (Graceful)
    };
...
function initBookingForm() {
  ...
  // Multiple days select buttons in Step 3
  ...
  // Reset booking form on successful submit reset
...
// ======================================================
// Testimonials Filter & Load More Handler
// ======================================================
function initTestimonials() {
  ...
  function updateVisibility() {
    ...
    // Count how many total matches there are for this filter
    const totalMatches = Array.from(cards).filter(card => {
      const category = card.getAttribute('data-category');
      return currentFilter === 'all' || category === currentFilter;
    }).length;

    // Show/hide Load More button depending on remaining count
  ...
  filterBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      limit = 6; // Reset page limit when filter changes
      updateVisibility();
    });
  });
...
// ======================================================
// Blog Filtering, Show More & Modal Reader Handler
// ======================================================
function initBlog() {
  ...
  if (loadMoreBtn) {
    loadMoreBtn.addEventListener('click', () => {
      limit += 4; // Reveal 4 more articles (showing remaining out of 10)
      updateBlogVisibility();
    });
  }
  ...
  cards.forEach(card => {
    ...
    readBtn.addEventListener('click', () => {
      ...
      // Parse calendar and read time from the meta children
      const metaSpanElements = card.querySelectorAll('.blog-card-meta span');
